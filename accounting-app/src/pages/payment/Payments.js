...
function Payments() {
  ...
  useEffect(() => {
    if (display === "default") {
      axios
        .get(
          `https://accounting.app-desk.com/payments/getPayment.php?id=${id}`
        )
  ...
  const submitHandler = (e) => {
    ...
    axios
      .post(
        `https://accounting.app-desk.com/payments/getPaymentFiltered.php?id=${id}`,
        formData,
        {
          headers: {
    ...
    <Container fluid className="mt-5">
      ...
      <Row className="mt-5">
        ...
        <Col lg="6" md="6" sm="6" xs="6">
          <Link className="m-1 float-end" to={`/clients/invoices/${id}`}>
            Invoices
          </Link>
          <Link className="m-1 float-end" to={`/clients/payments/addnew/${id}`}>
            Add Payment
          </Link>
        </Col>
      </Row>
      <Table>
        ...
        {display === "default" && (
          <tbody>
            {defaultPayments.map((payment, index) => (
              <tr key={index}>
