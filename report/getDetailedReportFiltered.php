...
$query = "
   SELECT DATE(expense_date) AS date, 
       ...
       NULL AS credit
FROM expenses
WHERE MONTH(expense_date) = '$month' AND YEAR(expense_date) = '$year' AND isDeleted = 0
UNION
SELECT DATE(date) AS date, 'Employee Salary' AS description, total_salary AS debit, NULL AS credit
FROM employees_salaries
WHERE MONTH(date) = '$month' AND YEAR(date) = '$year' AND isDeleted = 0
UNION
SELECT DATE(date) AS date, 'Loan Taken' AS description, loan_amount AS debit, NULL AS credit
FROM employees_loans
WHERE is_returned = 0 AND loan_amount > 0 AND MONTH(date) = '$month' AND YEAR(date) = '$year' AND isDeleted = 0
UNION
SELECT DATE(date) AS date, 'Loan Returned' AS description, NULL AS debit, loan_amount AS credit
FROM employees_loans
WHERE is_returned = 1 AND loan_amount > 0 AND MONTH(date) = '$month' AND YEAR(date) = '$year' AND isDeleted = 0
UNION
SELECT DATE(payment_date) AS date, 'Payment' AS description, NULL AS debit, payment_amount AS credit
FROM payments
WHERE payment_amount > 0 AND MONTH(payment_date) = '$month' AND YEAR(payment_date) = '$year' AND isDeleted = 0
ORDER BY date ASC
